...
// HID report descriptor parsing is OS-agnostic (it's just a byte format), so it lives
// here, shared by every backend that needs to resolve a device's usage page/usage id
// (currently `hidraw` on Linux and `uhid_bsd` on FreeBSD).
mod descriptor;
...
dyn_backend_impl! {
    ...
    // Not part of upstream async-hid: a FreeBSD backend built on the hidraw(4)
    // driver, added locally for dak's FreeBSD port. See
    // src/backend/hidraw_bsd/mod.rs.
    #[cfg(target_os = "freebsd")]
    mod hidraw_bsd {
        HidrawBsd(hidraw_bsd::HidrawBsdBackend)
    }
}
...
impl Default for DynBackend {
    ...
    fn default() -> Self {
        ...
        #[cfg(target_os = "freebsd")]
        {
            return Self::new(BackendType::HidrawBsd);
        }
