...
///
/// Shared between the Linux `hidraw` backend (which reads the descriptor bytes from
/// sysfs, see [`HidrawReportDescriptor::from_syspath`]) and the FreeBSD `uhid` backend
/// (which reads them via the `USB_GET_REPORT_DESC` ioctl, see
/// [`HidrawReportDescriptor::from_slice`]): the wire format of a HID report descriptor
/// is OS-agnostic, only how the raw bytes are obtained differs per platform.
#[derive(Default)]
