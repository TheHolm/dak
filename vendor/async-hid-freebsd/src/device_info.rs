...
pub enum DeviceId {
    ...
    /// Not part of upstream async-hid: added locally for dak's FreeBSD port. Holds
    /// the `/dev/hidrawN` character device path (there is no sysfs-like metadata
    /// directory to point at separately). Deliberately reuses the `DevPath` name
    /// Linux already uses so that downstream code matching on `DeviceId::DevPath`
    /// needs no platform-specific branch.
